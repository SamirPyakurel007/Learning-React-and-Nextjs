...
function getReviewCount(book) {
  const good = book.reviews.goodreads.reviewsCount;
  const librarything = book.reviews.librarything?.reviewsCount ?? 0;
  return good + librarything;
}
...
const y = [1, 2, 7, 4, 10, 3, 0, 9, 5];
const sorted = y.slice().sort((a, b) => a - b);
// console.log(sorted);
// console.log(y);

const sortPages = books
  .slice()
...
const removeBook = addBook.filter((book) => book.id !== 3);
// console.log(removeBook);

const updateBook = removeBook.map((book) =>
